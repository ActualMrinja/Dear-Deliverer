...
textmaker = function(text, x, y, size, sizeswitch = false) {
    ...
    for (let textsplit = 0; textsplit < text.split("\n").length; textsplit++) {
        ...
        if (sizeswitch) {
            ...
            ctx.strokeText(text.split("\n")[textsplit], (x+((window.devicePixelRatio == 1 ? 1 : window.devicePixelRatio*2)-1)) * (hs / 297) - (ctx.measureText(text.split("\n")[textsplit]).width / 2), (y + (textsplit * size * 1.25)) * (hs / 297), ctx.measureText(text.split("\n")[textsplit]).width);
            ...
            ctx.strokeStyle = "black";
            ctx.lineWidth = (size / 25) * ((24/size)+4) * (hs / 297);
            ctx.strokeText(text.split("\n")[textsplit], (x +((window.devicePixelRatio == 1 ? 1 : window.devicePixelRatio*2)-1)) * (hs / 297), (y + (textsplit * size * 1.25)) * (hs / 297), ctx.measureText(text.split("\n")[textsplit]).width * (hs / 297));
            ctx.fillStyle = "#ffffff";
